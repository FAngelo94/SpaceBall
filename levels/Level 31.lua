--In this World the margin became real body and defend the goal of someone
--in a random way
...
function setRequire()
	...
	energyPush=require(pathLogic.."EnergyPush")
...
function setup(groupGraph, comp, sl)
	...
	scene.ballTimeMin=4000
	scene.ballTimeMax=5500
	scene.maxBalls=5
	if selectedLevel==45 then
		scene.ballTimeMin=8000
		scene.ballTimeMax=10000
		maxEnemySpeed=10
		incrementEnemy=0
		scene.maxBalls=3
	end
...
function isGameEnd()
	...
	if enemy.finishGame() or (playerPoints==0 and selectedLevel==45) then
		scene.goToTheEnd()
...
function createBarrier()
	local ok=true
	while ok do
		local howManyPlayer=math.random(1, 4)
		if howManyPlayer==1 then
			ringG.marginBecomeBody("bot")
			ok=false
		elseif howManyPlayer==2 and enemy.getRightEnemy()~=nil then
			ringG.marginBecomeBody("right")
			ok=false
		elseif howManyPlayer==3 and enemy.getTopEnemy()~=nil then
			ringG.marginBecomeBody("top")
			ok=false
		elseif howManyPlayer==4 and enemy.getLeftEnemy()~=nil then
			ringG.marginBecomeBody("left")
			ok=false
		end
	end
	local delay=math.random(scene.delayMin, scene.delayMax)
	timerBarrier=timer.performWithDelay(delay, createBarrier, 1)
end
...
function scene:create( event )
	...
	timerBarrier=timer.performWithDelay(4000, createBarrier, 1)
	setupListener()
