--In this World the enemy is bigger than player
--The speed is egual for all 15 levels
...
local background
--Real level selected from 31 to 45
local selectedLevel
...
function createGraphicsAndLogic(groupGraph)
	...
	enemy.setup(groupGraph, scene, ball, ringG, 150)
end
...
function setup(groupGraph, comp, sl)
	...
	scene.maxBalls=5
	if selectedLevel==105 then
		scene.ballTimeMin=9000
		scene.ballTimeMax=11000
		maxEnemySpeed=10
		incrementEnemy=0
		scene.maxBalls=3
	end
	--How far the ball is when enemy follow it
	scene.distanceBallAction=300
	--Start points
...
function isGameEnd()
	...
	if enemy.finishGame() or (playerPoints==0 and selectedLevel==105) then
		scene.goToTheEnd()
