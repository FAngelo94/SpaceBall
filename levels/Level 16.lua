--In this World the ship can use the "Power Push" to push away with force
--the balls
--The speed become faster and faster
...
local speedStartEnemy=10
local incrementEnemy=0
local maxEnemySpeed=10
--Background to manage touch events
local background
--Real level selected from 16 to 30
local selectedLevel
...
function createGraphicsAndLogic(groupGraph)
	...
	energyPush.setup(groupGraph, scene, ball)
	energyPush.setupPlayer(player)
	energyPush.setupEnemy(enemy)
end
...
function setup(groupGraph, comp, sl)
	...
	scene.bossPoints=10
	--Setup energy push
	scene.rechargeEnergy=4000
	scene.bounceEnergy=5
	scene.rechargeEnemyEnergyMax=5000
	scene.rechargeEnemyEnergyMin=3000
...
function isGameEnd()
	...
	if enemy.finishGame() or (playerPoints==0 and selectedLevel==30) then
		scene.goToTheEnd()
...
function createTimers()
	...
	table.insert(timersTable,t1)
	t1=timer.performWithDelay(100, energyPush.activeEnemyEnergy,0)
	table.insert(timersTable,t1)
...
--Remove balls and pause buttons
