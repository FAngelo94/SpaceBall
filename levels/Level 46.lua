--In this World the shape of the balls change
...
function createGraphicsAndLogic(groupGraph)
	...
	--Setup Game Logic
	ball.setup(scene, ringG.getRing(), "differentShape")
...
function setup(groupGraph, comp, sl)
	...
	scene.ballTimeMin=3000
	scene.ballTimeMax=4500
	scene.maxBalls=5
	if selectedLevel==60 then
...
function isGameEnd()
	...
	if enemy.finishGame() or (playerPoints==0 and selectedLevel==60) then
		scene.goToTheEnd()
