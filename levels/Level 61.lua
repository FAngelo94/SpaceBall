--In this World the gravity change more times during the game
...
function setRequire()
	...
	--Require for arrow corner that indicate the gravity
	cornerArrow=require(pathGraphics.."CornerArrow")
end
...
function setup(groupGraph, comp, sl)
	...
	scene.ballTimeMin=2500
	scene.ballTimeMax=4000
	scene.maxBalls=5
	if selectedLevel==75 then
		scene.ballTimeMin=6500
		scene.ballTimeMax=8500
		maxEnemySpeed=10
		incrementEnemy=0
		scene.maxBalls=3
	end
...
function isGameEnd()
	...
	if enemy.finishGame() or (playerPoints==0 and selectedLevel==75) then
		scene.goToTheEnd()
...
--Change Gravity in random way
function changeGravity()
	--1=Left-Bot corner, 2=Right-Bot corner ecc..
	local forceOfGravity=20
	local position=math.random(1,4)
	if(position==1)then
		physics.setGravity(-forceOfGravity,forceOfGravity)
		cornerArrow.show(1)
	...
	timerGravity=timer.performWithDelay(delay, changeGravity, 1)
end
...
function scene:create( event )
	...
	timerGravity=timer.performWithDelay(1000, changeGravity, 1)
	setupListener()
...
function scene:hide( event )
    ...
    if ( phase == "will" ) then
		...
		--I delete all timer and remove all graphics
		physics.pause()
		removeListener()
		display.remove(background)
		cancelTimers()
		timer.cancel(timerGravity)
		removeGraphics()
    elseif ( phase == "did" ) then
